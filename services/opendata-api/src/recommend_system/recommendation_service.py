...
class RecommendationService:
    ...
    async def get_recommendations_from_cache(self, doc_id: str, top_k: int = 4) -> list[dict[str, Any]] | None:
        ...
        try:
            ...
            if result and result.recommendations:
                ...
                logger.info(f"캐시에서 추천 조회 완료: {doc_id} -> {len(recommendations)}개")
    ...
    def get_recommendations_realtime(self, doc_id: str, top_k: int = 4, threshold: float = 0.5) -> list[dict[str, Any]]:
        """Milvus를 사용한 실시간 추천"""
        try:
            ...
            logger.info(f"실시간 추천 완료: {doc_id} -> {len(recommendations)}개")
            return recommendations
    ...
    async def store_recommendations(self, doc_id: str, target_doc_type: str, recommendations: list[dict[str, Any]], ttl_days: int = 7):
        """추천 결과를 MongoDB에 저장"""
        try:
            ...
            if existing_rec:
                ...
                await new_rec.save()
                logger.info(f"추천 결과 저장 완료: {doc_id} -> {len(recommendations)}개")

        except Exception as e:
            logger.error(f"추천 결과 저장 실패: {e}")

    async def get_recommendations(self, doc_id: str, target_doc_type: str = "API", top_k: int = 4, use_cache: bool = True) -> list[dict[str, Any]]:
        """통합 추천 조회"""
        try:
            if use_cache:
                cached_recommendations = await self.get_recommendations_from_cache(doc_id, top_k)
                if cached_recommendations:
                    return cached_recommendations

            realtime_recommendations = self.get_recommendations_realtime(doc_id, top_k)
            if realtime_recommendations:
                await self.store_recommendations(doc_id, target_doc_type, realtime_recommendations)
    ...
    async def batch_generate_recommendations(self, doc_ids: list[str], target_doc_type: str = "API", top_k: int = 4):
        """배치로 추천 생성 및 저장"""
        ...
        for doc_id in doc_ids:
            try:
                recommendations = self.get_recommendations_realtime(doc_id, top_k)
                if recommendations:
                    await self.store_recommendations(doc_id, target_doc_type, recommendations)
                    success_count += 1

            except Exception as e:
                logger.error(f"문서 {doc_id} 추천 생성 실패: {e}")

        logger.info(f"배치 추천 생성 완료: {success_count}/{len(doc_ids)}개 성공")
