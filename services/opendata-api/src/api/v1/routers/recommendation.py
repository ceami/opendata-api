...
class RecommendationResponse(BaseModel):
    ...
    recommendations: list[RecommendationItem] = Field(..., description="추천 문서 리스트")
...
class RecommendationStatsResponse(BaseModel):
    ...
    avg_recommendations_per_doc: float = Field(..., description="문서당 평균 추천 수")
...
async def get_recommendations(
    ...
    target_doc_type: str = Query("API", description="대상 문서 타입 (API/FILE)"),
    service = Depends(get_recommendation_service)
):
    """문서 추천 조회"""
    try:
        logger.info(f"추천 요청: doc_id={doc_id}, top_k={top_k}, use_cache={use_cache}")
        recommendations = await service.get_recommendations(doc_id, target_doc_type, top_k, use_cache)
        cached_recommendations = service.get_recommendations_from_cache(doc_id, top_k)
        is_cached = cached_recommendations is not None and len(cached_recommendations) > 0
...
async def get_recommendations_realtime(
    ...
    """실시간 문서 추천 조회"""
    try:
        logger.info(f"실시간 추천 요청: doc_id={doc_id}, top_k={top_k}, threshold={threshold}")
        recommendations = service.get_recommendations_realtime(doc_id, top_k, threshold)
        recommendation_items = [
            RecommendationItem(
                doc_id=rec["doc_id"],
                similarity_score=rec["similarity_score"],
        ...
        raise HTTPException(status_code=500, detail=f"실시간 추천 조회 실패: {str(e)}")
...
async def get_recommendations_from_cache(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"캐시 추천 조회 실패: {str(e)}")
...
async def batch_generate_recommendations(
    ...
    """배치 추천 생성"""
    try:
        logger.info(f"배치 추천 생성 요청: {len(doc_ids)}개 문서, top_k={top_k}")
        if len(doc_ids) > 100:
            raise HTTPException(status_code=400, detail="한 번에 최대 100개 문서까지 처리 가능합니다")
        success_count = await service.batch_generate_recommendations(doc_ids, target_doc_type, top_k)
        ...
        raise HTTPException(status_code=500, detail=f"배치 추천 생성 실패: {str(e)}")
...
async def get_recommendation_stats(
    ...
    try:
        ...
        return RecommendationStatsResponse(
            ...
            avg_recommendations_per_doc=stats.get("avg_recommendations_per_doc", 0.0),
            cache_hit_ratio=stats.get("cache_hit_ratio", "N/A")
        )
    except Exception as e:
        logger.error(f"추천 통계 조회 실패: {e}")
        raise HTTPException(status_code=500, detail=f"추천 통계 조회 실패: {str(e)}")
...
async def clear_recommendation_cache(
    ...
    try:
        ...
        result = service.recommend_collection.delete_one({"target_doc_id": doc_id})

        if result.deleted_count > 0:
            return {"message": f"문서 '{doc_id}'의 추천 캐시가 삭제되었습니다"}
        else:
            return {"message": f"문서 '{doc_id}'의 추천 캐시를 찾을 수 없습니다"}
